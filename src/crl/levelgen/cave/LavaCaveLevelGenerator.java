...
public class LavaCaveLevelGenerator extends LevelGenerator{
	...
	public Level generateLevel(int xdim, int ydim, boolean locked, boolean xmirror) throws CRLException{
		...
		if (!xmirror){
			...
			intMap[intMap.length-1][yEntrance] = 0;
			for (int i = 1; i < 9; i++){
				intMap[intMap.length-1-i][yEntrance-1] = 0;
				intMap[intMap.length-1-i][yEntrance] = 0;
				intMap[intMap.length-2-i][yEntrance] = 0;
				intMap[intMap.length-1-i][yEntrance+1] = 0;
			}
			//Carve the exit
			intMap[0][yExit] = 0;
	...
	private Position temp = new Position(0,0);
