...
public class Rebound extends Action{
	...
	public void execute(){
        ...
        int runLength = 0;
        //DirectionMissileEffect x = new DirectionMissileEffect(aPlayer.getPosition(), "\\|/--/|\\", Appearance.BLUE, targetDirection, 20, 15);
        ...
        for (int i=0; i<20; i++){
			...
			if (targetCell != null && (targetCell.isSolid() || targetCell.getHeight() > aLevel.getMapCell(performer.getPosition()).getHeight()+2)){
				...
				if (aLevel.getMapCell(runner.x + var.x, runner.y, performer.getPosition().z) == null || aLevel.getMapCell(runner.x + var.x, runner.y, performer.getPosition().z).isSolid())
					bounce.x = 1;
				if (aLevel.getMapCell(runner.x, runner.y + var.y, performer.getPosition().z) == null || aLevel.getMapCell(runner.x, runner.y + var.y, performer.getPosition().z).isSolid())
					bounce.y = 1;
/*				if (aLevel.getMapCell(runner.x, runner.y + var.y, performer.getPosition().z) == null || aLevel.getMapCell(runner.x+var.x, runner.y + var.y, performer.getPosition().z).isSolid()){
					bounce.y = 1;
					bounce.x = 1;
				}                                         */
