...
public class BallOfDestruction extends Action{
	...
	public void execute(){
		...
		if (i == 20)
			//drawEffect(new MissileEffect(new Position(aPlayer.getPosition()), "*~", Appearance.RED, targetDirection,20, true, false));
			drawEffect(EffectFactory.getSingleton().createDirectedEffect(aPlayer.getPosition(), this.getPositionalDirectionFrom(aPlayer.getPosition()), "SFX_RENEGADE_BOD", 20));
		//-----
		i=0;
		for (; i<20; i++){
			Position destinationPoint = Position.add(aPlayer.getPosition(),
		...
		if (i == 20)
			//drawEffect(new MissileEffect(new Position(aPlayer.getPosition()), "*~", Appearance.RED, otherDir1,20, true, false));
			drawEffect(EffectFactory.getSingleton().createDirectedEffect(aPlayer.getPosition(), this.getPositionalDirectionFrom(aPlayer.getPosition(), otherDir1), "SFX_RENEGADE_BOD", 20));
				//-----
		i=0;
		for (; i<20; i++){
			Position destinationPoint = Position.add(aPlayer.getPosition(),
		...
		if (i == 20)
			//drawEffect(new MissileEffect(new Position(aPlayer.getPosition()), "*~", Appearance.RED, otherDir2,20, true, false));
			drawEffect(EffectFactory.getSingleton().createDirectedEffect(aPlayer.getPosition(), this.getPositionalDirectionFrom(aPlayer.getPosition(), otherDir2), "SFX_RENEGADE_BOD", 20));
	...
	private boolean hit (Position destinationPoint, int i){
        ...
        if (destinationFeature != null && destinationFeature.isDestroyable()){
	       	...
	       	//drawEffect(new MissileEffect(new Position(aPlayer.getPosition()), "*~", Appearance.RED, targetDirection,i, true, false));
	       	drawEffect(EffectFactory.getSingleton().createDirectedEffect(aPlayer.getPosition(), this.getPositionalDirectionFrom(aPlayer.getPosition()), "SFX_RENEGADE_BOD", i));
